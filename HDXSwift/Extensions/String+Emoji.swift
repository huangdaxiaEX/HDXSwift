...
extension String {
    ...
    public var emojiUnescapedString: String {
        ...
        let ms = String.emojiUnescapeRegExp.matchesInString(self, options: [], range: NSRange(location: 0, length: s.length))
        ms.reverse().forEach { m in
            let r = m.range
            let p = s.substringWithRange(r)
            let px = p.substringWithRange(Range<String.Index>(p.startIndex.successor()...p.endIndex.predecessor()))
            if let t = emoji[px] {
    ...
    public var emojiEscapedString: String {
        var s = self as NSString
        let ms = String.emojiEscapeRegExp.matchesInString(self, options: [], range: NSRange(location: 0, length: s.length))
        ms.reverse().forEach { m in
            let r = m.range
            let p = s.substringWithRange(r)
